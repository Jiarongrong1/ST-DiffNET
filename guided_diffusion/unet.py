...
from .SMSPCNN import improved_pcnn_dynamic_alpha
...
@torch.no_grad()
def pcnn_postprocess_torch(img_tensor, t=60):
    result = []
    for img in img_tensor:  # 遍历 batch
        img_np = img.squeeze(0).cpu().numpy()  # [1, H, W] -> [H, W]
        enhanced, _ = improved_pcnn_dynamic_alpha(img_np, t)
        result.append(torch.tensor(enhanced).unsqueeze(0))  # [1, H, W]

    return torch.stack(result).to(img_tensor.device)  # [B, 1, H, W]

class TimestepBlock(nn.Module): # 该方法接受两个参数：x和emb。其中，x是需要处理的数据，emb是时间步嵌入。
    """
...
#上采样有两种：转置卷积，upsampling+conv。下面这个用的是upsampling+conv，用if语句判要不要conv
...
class UNetModel_v1preview(nn.Module):
    ...
    def pcnn_postprocess_torch(self, img_tensor, t=60):
        result = []
        with torch.no_grad():
            for img in img_tensor:  
                img_np = img.squeeze(0).detach().cpu().numpy()  # [C, H, W] → numpy
                enhanced = improved_pcnn_dynamic_alpha(img_np, t)  # ✅ 只返回一个值！
                if isinstance(enhanced, np.ndarray):
                    enhanced_tensor = torch.tensor(enhanced).unsqueeze(0)  # [1, C, H, W]
                    result.append(enhanced_tensor)
        return torch.cat(result, dim=0).to(img_tensor.device)
    ...
    def forward(self, x, timesteps, y=None):
        ...
        out = self.out(h)
        with th.no_grad():  
            out = self.pcnn_postprocess_torch(out.detach())
        return out, cal
...
class MultiScaleAttention(nn.Module):
    def __init__(self, in_channels, scales=[1, 2, 4]):
        ...
        for scale in scales:
            self.attention_convs.append(nn.Sequential(
                nn.Conv2d(in_channels, in_channels, kernel_size=3, stride=1, padding=scale, dilation=scale),
                nn.ReLU(inplace=True),
                nn.Conv2d(in_channels, 1, kernel_size=1)
            ))
        self.global_conv = nn.Conv2d(len(scales), in_channels, kernel_size=1)
    ...
    def forward(self, x):
        ...
        attention_maps = torch.cat(attentions, dim=1)
        global_attention = self.global_conv(attention_maps)
...
class DCG(nn.Module):
    def __init__(self, parameters):
        ...
        # 构建网络
        self.multi_scale_attention = MultiScaleAttention(in_channels=3, scales=[1, 2, 4])
    ...
    def _retrieve_crop(self, x_original_pytorch, crop_positions, crop_method):
        ...
        return output
    def forward(self, x):
        x_original_pytorch = x
        # device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        # x_original_pytorch = x_original_pytorch.to(device)

        # 提取全局特征
        multi_scale_features = self.multi_scale_attention(x_original_pytorch)
        h_g, self.saliency_map = self.global_network.forward(multi_scale_features)

        # 提取局部特征
        small_x_locations = self.retrieve_roi_crops.forward(x, self.cam_size, self.saliency_map)
        self.patch_locations = self._convert_crop_position(small_x_locations, self.cam_size, x)
        crops_variable = self._retrieve_crop(x_original_pytorch, self.patch_locations, self.retrieve_roi_crops.crop_method)
        crops_variable = F.interpolate(crops_variable, size=(256, 256), mode='bilinear', align_corners=False)
        multi_scale_features0 = self.multi_scale_attention(crops_variable)
        h_crops, self.saliency_map = self.global_network.forward(multi_scale_features0)
...
class Parameters:
    ...
    class Data:
        def __init__(self):
            self.num_classes = 3  # 示例，设置正确的 num_classes 值
...
class ChannelAttention(nn.Module):
    def __init__(self, channels, reduction_ratio=16):
        ...
        self.fc = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Conv2d(128, 2048, kernel_size=1, bias=False)
        )
...
# 修改的 Cross Attention 模块，前面加上 CBAM
class Cross_Attention(nn.Module):
    def __init__(self):
        super(Cross_Attention, self).__init__()
        self.cbam = CBAM(2048)  # 输入通道数为 2048
        # 通道降维
    ...
    def forward(self, x_s, x_t):
        ...
        if x_t.dim() == 4:
            ...
            raise ValueError("Unexpected tensor shape: {}".format(x_t.shape))

        # 减少通道数
        x_s_reduced = self.reduce_channels(x_s)  # (B, 512, W, H)
        x_t_reduced = self.reduce_channels(x_t)  # (B, 512, W, H)

        # 计算 query, key, value
        proj_query = self.query_conv(x_t_reduced).view(B, 64, W * H).permute(0, 2, 1)  # (B, W*H, 64)
        proj_key = self.key_conv(x_s_reduced).view(B, 64, W * H)  # (B, 64, W*H)
        proj_value = self.value_conv(x_s_reduced).view(B, 512, W * H)  # (B, 512, W*H)
        ...
        # 应用注意力机制到 value
        out = torch.bmm(proj_value, attention.permute(0, 2, 1))  # (B, 512, W*H)
        out = out.view(B, 512, W, H)  # 调整输出形状为 (B, 512, W, H)

        # 残差连接并应用 gamma 权重
        out = self.gamma * out + x_s_reduced

        return out


class Cross_Attention0(nn.Module):
    def __init__(self):
        super(Cross_Attention0, self).__init__()
...
class UNetModel_newpreview(nn.Module):
    ...
    def __init__(
        ...
        # 添加
        self.encoderd = DCG(parameters)
        self.encoderm = DCG(parameters)
        self.cross_attention = Cross_Attention()
        self.cross_attention0 = Cross_Attention()
        self.cross_attention1 = Cross_Attention0()
        #
        time_embed_dim = model_channels * 4
        self.time_embed = nn.Sequential(
            linear(model_channels, time_embed_dim),
    ...
    def pcnn_postprocess_torch(self, img_tensor, t=60):
        ...
        with torch.no_grad():
            for img in img_tensor:  
                img_np = img.squeeze(0).detach().cpu().numpy()  # [C, H, W] → numpy
                enhanced = improved_pcnn_dynamic_alpha(img_np, t) 
                if isinstance(enhanced, np.ndarray):
                    enhanced_tensor = torch.tensor(enhanced).unsqueeze(0)  # [1, C, H, W]
                    result.append(enhanced_tensor)
        return torch.cat(result, dim=0).to(img_tensor.device)
    ...
    def forward(self, x, timesteps, y=None):
        ...
        img_truth = h[:, 0:3, :, :]  # 提取 img_truth，形状为 (64, 3, 64, 64)
        img_m = h[:, 3:6, :, :]  # 提取 img_m，形状为 (64, 3, 64, 64)
        img_s = h[:, 6:9, :, :]  # 提取 img_s，形状为 (64, 3, 64, 64)
        mask = h[:, 9:12, :, :]
        c = th.cat((img_m, img_s), dim=1)  # 在通道维度进行了拼接，改变的是通道数
        c1 = img_m
        c2 = img_s
        h = th.cat((img_truth, img_m), dim=1)
        anch, cal = self.highway_forward(c)
        # 提取 encoderd 的特征
        t_y_global, t_y_local = self.encoderd(c1)
        # 提取 encoderm 的特征
        s_y_global, s_y_local = self.encoderm(c2)
        #
        for ind, module in enumerate(self.input_blocks):
            if len(emb.size()) > 2:
                emb = emb.squeeze()
        ...
        h = self.middle_block(h, emb)
        # 融合全局特征
        global_fusion = self.cross_attention(s_y_global, t_y_global)
        # 融合局部特征
        local_fusion = self.cross_attention0(s_y_local, t_y_local)

        self.kl_g_s = 0
        self.kl_g_t = 0

        distribution1 = self.cross_attention1(global_fusion, local_fusion)
        h = h + distribution1
        ...
        h = h.type(x.dtype)
        out = self.out(h)
        return out, cal  # , self.kl_g_s, self.kl_g_t
        # 添加
        # fengbu = []
        # fengbu0 = []

        # , self.kl_g_s, self.kl_g_t
        # q_mu, q_std = torch.split(distribution, 256, dim=1)
        # fengbu.append([q_mu, F.softplus(q_std)])
        #
        # q_mu0, q_std0 = torch.split(distribution0, 256, dim=1)
        # fengbu0.append([q_mu0, F.softplus(q_std0)])
        #
        # self.kl_g_s = self.get_distribution(fengbu)
        # self.kl_g_t = self.get_distribution(fengbu0)

        # distribution1 = self.cross_attention1(distribution, distribution0)
        # mu, sigma = torch.split(distribution1, 256, dim=1)
        # distribution_normal = torch.distributions.Normal(mu, F.softplus(sigma))
        # z = distribution_normal.rsample()
        # h = h + th.cat([z, z], dim=1)
        # h = h+distribution1
...
class Generic_UNet(SegmentationNetwork):
    ...
    def __init__(self, input_channels, base_num_features, num_classes, num_pool, num_conv_per_stage=2,
        ...
        if self._deep_supervision:
            ...
            self.seg_outputs.append(conv_op(self.conv_blocks_localization[-1][-1].output_channels, 3,
                            1, 1, 0, 1, 1, seg_output_use_bias))
