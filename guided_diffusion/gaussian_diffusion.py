...
class GaussianDiffusion:
    ...
    def save_results(self, save_data, out_dir, score=None, data_name='none'):
        ...
        for i in range(save_data.size(0)):
            ...
            if type(score) == type(None):
                img_name = '%s_%s.png' % (data_name, i)
            else:
                img_name = '%s_%s_%s.png' % (data_name, i, str(score))
    ...
    def p_mean_variance(
        ...
        self.save_results(model_output, r"XXXXXXXXX", data_name='model_output')


        x=x[:,0:3,...]  #loss is only calculated on the last channel, not on the input brain MR image
    ...
    def sample_known(self, img, batch_size = 1, model=None):
    ...
    def p_sample_loop_known(
        ...
        img = img.to(device)
        # noise = th.randn_like(img[:, 0:3, ...]).to(device)
        # x_noisy = torch.cat((img[:, 0:3,  ...], noise), dim=1)  #add noise as the last channel
        x_noisy = img
        ...
        if self.dpm_solver:
            ...
            return final["sample"], x_noisy, img, final["cal"], final["pred_xstart"]
    ...
    def p_sample_loop_progressive(
        ...
        org_c = img.size(1)
        org_MRI = img[:, 3:12, ...]      #original brain MR image
        if progress:
           ...
           for i in indices:
                ...
                with th.no_grad():
                    ...
                    if img.size(1) != org_c:
                        img = torch.cat((img, org_MRI), dim=1)       #in every step, make sure to concatenate the original image to the sampled segmentation mask
